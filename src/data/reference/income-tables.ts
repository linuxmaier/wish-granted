 ...
 *
 * Verified 2026-08-21 against the LIHEAP Clearinghouse's "Wisconsin State
 * Median Income for FFY 2026" page
 * (https://liheapch.acf.gov/profiles/povertytables/FY2026/wismi.htm), an
 * HHS/ACF Office of Community Services republication that prints the six
 * household-size-1-6 dollar figures directly: 38_421 / 50_243 / 62_065 /
 * 73_888 / 85_710 / 97_532 -- an exact match to bySize[0..5] below. The page
 * itself carries a publisher-authored "[Last edited 12/05/2025]" stamp --
 * unlike the generic Last-Modified header noise the issue #5 spike found
 * unusable elsewhere, an editorially-maintained date stamp on a federal page
 * is a real change-detection signal, worth issue #7 building around.
 *
 * Three independent routes now agree on these figures: #3's original
 * `pdftotext` extraction from Appendix E ("PY26 (2025-2026) Wisconsin 60%
 * State Median Income Guidelines") of the WHEAP PY26 Manual (a 2.5 MB PDF,
 * revised August 2025), this Clearinghouse HTML page, and 45 CFR 96.85(b)'s
 * fixed percentages applied to the fetched four-person base (below). PY26
 * runs October 1, 2025 - September 30, 2026, so it is the program year in
 * effect today.
 *
 * `source` below points at the Clearinghouse page rather than the WHEAP PDF:
 * it is plain HTML, not a 2.5 MB PDF that needs `pdftotext`, so it is what
 * the issue #6 automated refresher actually fetches, and it is the URL these
 * figures were verified against. See docs/data-authoring.md, "Automated
 * refresh", for why the refresher no longer touches the PDF for this table.
 *
 * `perAdditionalPerson` (for household sizes 9+, beyond the published table)
 * is not stated in either source, but is not a guess either: 45 CFR 96.85, the
