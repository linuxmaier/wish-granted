    ...
    <div className="app">
      ...
      */}
      ...
      <footer className="footer">
        ...
        {/*
          The "data as of" provenance line (issue #44). Deliberately down here in
          the footer's quiet type, not up by the masthead: it is provenance, not
          a headline, and it must not compete with results for attention. It says
          two separate things on purpose -- when the dataset was assembled, and
          how much of it is currently verified -- because a build date is not a
          verification date and collapsing them would mislead. See
          DataProvenance.tsx. It sits below, and reads quieter than, the
          unverified-data banner; that banner's behaviour is untouched.
        */}
        <DataProvenance />

        <p>
          {/* target="_blank" is deliberate: there's no persistence by design (see
              docs/design.md, "Privacy architecture"), so navigating away in the same tab
              would erase an in-progress interview. The aria-label announces the new tab
              explicitly so that isn't a surprise to assistive tech users -- do not remove
              target="_blank" to "fix" this; the surprise is the actual bug, not the tab. */}
          <a
            href="/privacy.html"
            target="_blank"
            rel="noopener"
