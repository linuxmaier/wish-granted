import { FACTS, RESERVED_FACT_KEYS } from '../../src/domain/facts.ts';
...
export function gateCriterion(criterion: Criterion): GateResult {
  ...
  for (const node of walk(criterion)) {
    if ((node.kind === 'compare' || node.kind === 'set') && RESERVED_FACT_KEYS.includes(node.fact)) {
      problems.push(
        `${node.kind} references "${node.fact}", a reserved fact the interview never asks -- a rule testing it can never be satisfied and would silently rule everyone out. Route this condition to a manualReview note instead.`,
      );
    }
